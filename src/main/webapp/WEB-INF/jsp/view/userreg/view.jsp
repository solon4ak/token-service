...
<template:basic htmlTitle="User page for ${user.lastName}, ${user.firstName}"
    ...
    <br /><br />
    <a href="<c:url value="/user/edit" />">Edit user</a>
    <hr />
    <c:choose>
        <c:when test="${user.postAddress == null}">
            <a href="<c:url value="/token/user/postaddress/add" />">Add post address</a>
        </c:when>
        <c:otherwise>
            <h4>Post Address:</h4>
            <table class="data_table">
            ...
            <br />
            <a href="<c:url value="/token/user/postaddress/edit" />">Edit post address</a>
        </c:otherwise>
    </c:choose>
    <hr />
    <c:choose>
